...
bool LinkedBag<ItemType>::removeSecondNode340() {
	...
	if (canRemoveItem) {
		...
		std::unique_ptr<Node<ItemType>> deleteSmart{ nodeToDeletePtr };
		headPtr = headPtr->getNext();

		deleteSmart->setNext(nullptr);
...
int LinkedBag<ItemType>::getCurrentSize340RecursiveHelper(Node<ItemType>* currentPtr) const {
	...
	if (currentPtr->getNext() != nullptr) {
		sizeCount1 += getCurrentSize340RecursiveHelper(currentPtr->getNext());
		return sizeCount1;
	}
	return 1;
}
...
int LinkedBag<ItemType>::getCurrentSize340Recursive() const {
	...
	int sizeCount = 1;//not empty
	sizeCount += getCurrentSize340RecursiveHelper(headPtr->getNext());
	return sizeCount;
}
...
int LinkedBag<ItemType>::getFrequencyOf340RecursiveHelper(Node<ItemType>* nowNode, const ItemType& item) const {
	static int frequency;//make global
